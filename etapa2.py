...
class Conexao:
    def __init__(self, id_conexao, seq_inicial, ack_inicial):
        self.id_conexao = id_conexao
        self.seq_no = seq_inicial
        self.ack_no = ack_inicial
        self.seq_inicial = seq_inicial
        self.ack_inicial = ack_inicial
...
def send_next(fd, conexao):
    ...
    endpayload = conexao.seq_no + MSS - conexao.seq_inicial - 1
    startpayload = conexao.seq_no - conexao.seq_inicial - 1

    if (startpayload >= len(conexao.send_queue)):
        payload = b""
    elif (endpayload >= len(conexao.send_queue)):
        payload = conexao.send_queue[startpayload:]
    else:
        payload = conexao.send_queue[startpayload:endpayload]
    ...
    if not TESTAR_PERDA_ENVIO :
        fd.sendto(segment, (dst_addr, dst_port))
        print("SENT seq: %d (%d) ack: %d (%d)" % (
            conexao.seq_no,  conexao.seq_no - conexao.seq_inicial,  conexao.ack_no,  conexao.ack_no - conexao.ack_inicial))
...
def raw_recv(fd):
    ...
    if (flags & FLAGS_SYN) == FLAGS_SYN:
        ...
        new_seq = struct.unpack('I', os.urandom(4))[0]
        conexoes[id_conexao] = conexao = Conexao(id_conexao=id_conexao, seq_inicial=new_seq, ack_inicial=seq_no + 1)
        print("start con", "seq:", conexao.seq_no, "ack:", conexao.ack_no)
        #envia syn ack
        ...
        print("\nRECEIVED seq: %d (%d) ack: %d (%d)" % (seq_no, seq_no - conexao.ack_inicial, ack_no, ack_no - conexao.seq_inicial))
        conexao.seq_no = ack_no
